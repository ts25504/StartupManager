...
void ListItems::ReadDisabledItemsFromFile(std::vector<ValueInfo>& vi_vec)
{
    ...
    for (ULONG i = 0; i < num_of_disabled_items; ++i)
    {
        ...
        vi.sz_value_name[wcslen(vi.sz_value_name)-1] = TEXT('\0');
        fgetws(vi.sz_value, MAX_VALUE, fp);
        vi.sz_value[wcslen(vi.sz_value)-1] = TEXT('\0');
        fgetws(sz_key, MAX_KEY_LENGTH, fp);
        sz_key[wcslen(sz_key)-1] = TEXT('\0');
        fgetws(vi.sz_subkey, MAX_KEY_LENGTH, fp);
        vi.sz_subkey[wcslen(vi.sz_subkey)-1] = TEXT('\0');
        fgetws(vi.sz_product_name, MAX_PATH, fp);
        vi.sz_product_name[wcslen(vi.sz_product_name)-1] = TEXT('\0');
        fwscanf_s(fp, TEXT("%d\n"), &vi.state);
        if(wcscmp(sz_key, TEXT("HKEY_CLASSES_ROOT")) == 0)
            vi.h_key = HKEY_CLASSES_ROOT;
        else if (wcscmp(sz_key, TEXT("HKEY_CURRENT_USER")) == 0)
            vi.h_key = HKEY_CURRENT_USER;
        else if (wcscmp(sz_key, TEXT("HKEY_LOCAL_MACHINE")) == 0)
            vi.h_key = HKEY_LOCAL_MACHINE;
        else if (wcscmp(sz_key, TEXT("HKEY_USERS\n")) == 0)
            vi.h_key = HKEY_USERS;
        else if (wcscmp(sz_key, TEXT("HKEY_CURRENT_CONFIG")) == 0)
    ...
    for (ULONG i = 0; i < vi_vec.size(); ++i)
    {
        if (vi_vec[i].state == 0)
        {
            if(vi_vec[i].h_key == HKEY_CLASSES_ROOT)
                wcscpy_s(sz_key, MAX_KEY_LENGTH, TEXT("HKEY_CLASSES_ROOT"));
            else if (vi_vec[i].h_key == HKEY_CURRENT_USER)
                wcscpy_s(sz_key, MAX_KEY_LENGTH, TEXT("HKEY_CURRENT_USER"));
            else if (vi_vec[i].h_key == HKEY_LOCAL_MACHINE)
                wcscpy_s(sz_key, MAX_KEY_LENGTH, TEXT("HKEY_LOCAL_MACHINE"));
            else if (vi_vec[i].h_key == HKEY_USERS)
                wcscpy_s(sz_key, MAX_KEY_LENGTH, TEXT("HKEY_USERS"));
            else if (vi_vec[i].h_key == HKEY_CURRENT_CONFIG)
                wcscpy_s(sz_key, MAX_KEY_LENGTH, TEXT("HKEY_CURRENT_CONFIG"));
...
void EnumFiles(std::vector<ValueInfo>& vi_vec, wchar_t* p_dir)
{
    ...
    while (b_not_finish)
    {
        if (wcscmp(file_data.cFileName, TEXT(".")) == 0 ||
            ...
            wcscmp(file_data.cFileName, TEXT("desktop.ini")) == 0)
        {
            b_not_finish = ::FindNextFile(h_search, &file_data);
            continue;
        }
        swprintf_s(sz_path, TEXT("%s\\%s"), p_dir, file_data.cFileName);
        ValueInfo vi;
        vi.h_key = NULL;
...
void ListItems::ResetItem(ValueInfo& vi)
{
    ...
    else
    {
        ...
        swprintf_s(sz_exist_path, TEXT("%s\\%s"), sz_folder_name, vi.sz_value_name);
